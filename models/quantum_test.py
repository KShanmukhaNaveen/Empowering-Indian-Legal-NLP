...
epoch_to_load = 10  # For example, if you want to load the model saved after the first epoch

# Construct the path to the saved model
model_path = os.path.join(save_dir, f"model_epoch_{epoch_to_load}.pt")
...
# running_corrects = 0
# for step,batch in enumerate(dataloaders["validation"]):
...
# epoch_acc = running_corrects / len(datasets["validation"])
# print(f"Phase: Validation /{num_epochs} Acc: {epoch_acc:.4f}")
...
for step, batch in enumerate(dataloaders["validation"]):
    ...
    if step % 32 == 0:
        elapsed = format_time(time.time() - t0)
        print(f'  Batch {step:>5,}  of  {len(validation_dataloader):>5,}.    Elapsed: {elapsed:}.')
    b_input_ids = batch['input_ids'].to(device)
    b_labels = batch['labels'].to(device)
    b_attention_masks = batch['attention_mask'].to(device)
    # optimizer.zero_grad()
